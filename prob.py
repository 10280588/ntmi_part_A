...
class Prob():
    ...
    def calculateProb(self, ngram, ngramMin1, n):
        print 'testo'
        print ngram
        print ngramMin1
        f = open(self.probfile, 'r')
        for line in f:
            ...
            strList = line.split()
            if len(strList) == n: # Doesn't print a lot when low n is chosen with text file with long lines (only lines of length n).
